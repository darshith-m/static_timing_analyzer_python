'''Python file to parse .bench files'''
import argparse
...
def read_ckt(path):
    ...
    for line in path.open():
        ...
        if node in ("INPUT", "OUTPUT"):
            ...
            for i in inputs:
                # If the node is not present in dictionary, create it
                if i not in circuit:
                    circuit[i] = Gates()
    ...
    result_file = open("../output/ckt_details.txt", "w", encoding="utf-8")
...
def read_nldm(path, delay_b=False, slews_b=False):
    ...
    for line in path.open():
        ...
        if gate_count > 0:
            ...
            if input_capacitance is not None:
            ...
            if input_slew_string is not None:
            ...
            if output_load_string is not None:
    ...
    if delay_b:
        result_file_delay = open("../output/delay_LUT.txt", "w", encoding="utf-8")
    # Create slews file if delays are enabled in arguments
    if slews_b:
        result_file_slew = open("../output/slew_LUT.txt", "w", encoding="utf-8")
    for value in gates.values():
        # Print delay values if delay argument is present
        if delay_b:
            result_file_delay.write(f"cell: {value.cell_name}\n")
            input_slews = ','.join([str(i) for i in value.input_slew])
            load_capacitance = ','.join([str(i) for i in value.output_load])
            result_file_delay.write(f"input slews: {input_slews}\n")
            result_file_delay.write(f"load cap: {load_capacitance}\n\n")
        ...
        if slews_b:
            ...
            input_slews = ','.join([str(i) for i in value.input_slew])
            load_capacitance = ','.join([str(i) for i in value.output_load])
            result_file_slew.write(f"input slews: {input_slews}\n")
            result_file_slew.write(f"load cap: {load_capacitance}\n\n")
    ...
    if delay_b:
        result_file_delay.close()
    # Close the slews file
    if slews_b:
        result_file_slew.close()
...
def parse_arguments():
    ...
    parser.add_argument("--delays", action = "store_true", help = "Provides cell delay.")
    # Argument to #print slews
    parser.add_argument("--slews", action = "store_true", help = "Provides cell slews.")
...
def main():
    '''Main function to parse .bench file'''
    ...
    if inputs.read_nldm is not None:
        ...
        if path_lib.exists():
            ...
            # pass delays and slews arguments to the function
            read_nldm(path_lib, inputs.delays, inputs.slews)
        else:
            print(".lib file doesn't exist.")
